...
function App() {
    ...
    if (maxPowerStation) {
      topStationListItems.push(
        <li>
          ...
          is the station at{' '}
          <strong>
            ( {maxPowerStation.x}, {maxPowerStation.y} )
          </strong>{' '}
          with power <strong>{maxPower.toFixed(2)}</strong>
        </li>
      );
    } else {
      topStationListItems.push(
  ...
  // NOTE: the ugly {' '} spaces were inserted by Prettier due to JSX; unfortunately, they can't be disabled :(
  const linkStationListItems = linkStations.map((station, index) => (
    <li key={index}>
      Station #{index + 1}:{' '}
      <strong>
  ...
  const pointListItems = points.map((point, index) => (
    <li key={index}>
      Point #{index + 1}:{' '}
      <strong>
        ( x: {point.x}, y: {point.y} )
